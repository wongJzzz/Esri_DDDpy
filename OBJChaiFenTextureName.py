...
def changeImgae(folder,ext): 
    ...
    for ar in arr:
        with Image(filename=ar) as im:
            ...
            im.rotate(90)
            im.transpose()
            im.save(filename=folder+fname+".png")
...
def mtlChaifen(mtlfile,imagePath):
    ...
    for i in range(0,len(wrlLines)):
        ...
        if wrlstring[0:6]=='newmtl':
            ...
            wrlLines[i]=wrlLines[i].replace('tga','png')
        elif wrlstring[wl-4:wl]=='.TGA':
            wrlLines[i]=wrlLines[i].replace('TGA','png')
...
def OBJSplit():
    ...
    OBJFilePath=r'C:\Users\wangjieth\Desktop\泰州模型OBJ\62050225max--\test.obj'
    imagePath="C:\\Users\\wangjieth\\Desktop\\泰州模型OBJ\\62050225max--\\"  #贴图地址
    changeImgae(imagePath,"tga")
    DeswrlFolder=os.path.dirname(OBJFilePath)
    if exists (OBJFilePath[:-3]+'mtl'):
        mtlList=mtlChaifen(OBJFilePath[:-3]+'mtl',imagePath)
